...
I clienti IBM possono accedere al servizio Logpull negli account Enterprise. Questo servizio consente agli utenti di utilizzare i log delle richieste su HTTP tramite una [CLI](/docs/cis-cli-plugin?topic=cis-cli-plugin-cis-cli-commands#log).

Questi log contengono dati correlati al client collegato, il percorso della richiesta attraverso la rete e la risposta dal server web di origine. 
...
Se un utente riceve un messaggio di errore dopo aver eseguito un comando, può utilizzare il RayID fornito nell'intestazione della risposta per ottenere i log relativi al comando. 

Se hai un RAY_ID con `-XXX` alla fine, assicurati di rimuovere questi caratteri. Ad esempio, `12ab34cdef567gh8-XXX` diventa `12ab34cdef567gh8`.
{.note}
...
Se un utente riceve un messaggio di errore dopo aver eseguito un comando ma non conosce il RayID della risposta, può utilizzare invece un periodo di tempo. Riceve tutti i log di una determinata zona che rientrano nella finestra temporale durante la quale si è verificato l'errore e può effettuare una ricerca tra i risultati. 


**Parametri obbligatori**

L'ora di inizio e quella di fine espresse come data/ora unix (in secondi o nanosecondi) oppure una data/ora assoluta conforme a RFC 3339, con un periodo di tempo di un minuto o di un'ora. 
...
Se i `campi` non vengono specificati nella richiesta, viene restituita una serie limitata di campi predefiniti. Trova l'elenco completo di tutti i campi disponibili qui: 
...
I campi vengono passati come un elenco separato da virgole. Ad esempio, per avere "ZoneID" e "RayID", utilizza:
...
Campi attualmente disponibili (a partire da dicembre 2018):
```
...
Di seguito vengono riportati una chiamata `logpull` di esempio ed esempi di risposte di tipi specifici. 
...
**Risposta con codice di stato 200**
```
{
...
**Risposta con codice di stato 404**
```
{
...
**Richiesta che corrisponde a una regola WAF (attacco SQLj)**
```
{
"CacheCacheStatus":"unknown",
...
**Richiesta che corrisponde a una regola firewall**
```
{
"CacheCacheStatus":"unknown",
"CacheResponseBytes":0,
...
**La richiesta era a frequenza limitata**
```
{
"CacheCacheStatus":"unknown",
"CacheResponseBytes":0,
...
**Il server di origine è inattivo (Error 521, Web Server is down)**
```
{
"CacheCacheStatus":"miss",
