...
新增 CF-RAY 標頭是為了協助透過網路追蹤對網站的要求。當您使用「支援中心」來協助對連線功能的任何相關問題進行疑難排解時，請使用它。透過在 Apache 及 nginx 的配置檔中進行一些編輯，您可以在日誌中顯示這個 "Ray ID"。
...
* 以下是[要列入白名單的 IP](/docs/infrastructure/cis?topic=cis-ibm-cloud-cis-whitelisted-ip-addresses) 清單
...
* 將問題單中 `www.YOUR_DOMAIN.com/cdn-cgi/trace` 的結果傳送給[支援中心](/docs/get-support?topic=get-support-getting-customer-support)。
* 暫時關閉 DNS 記錄（無 Proxy）。
