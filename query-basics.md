...
There are several basic aspects of querying with GraphQL for metrics collection.
{: shortdesc}
...
* `zones` or `accounts` - Indicate the scope of the query (the domain area or account you want to query). The `viewer` can access one `zones` or `accounts`, or both.
* **data node** or **dataset** - Represent the data you want to query. `zones` or `accounts` might contain one or more datasets.
...
In this query, there are variable placeholders: `$tag`, `$start`, and `$end`. It provides values for those placeholders alongside the query by placing them into the `variables` field of the payload.
...
A sample of a response for this query:
